...
local function giveKeyssamy(source, plate)
    local vehicles = plate and GetVehiclesFromPlate(plate) or {GetVehiclePedIsIn(GetPlayerPed(source), false)}
    local success = false
    for i = 1, #vehicles do
        local vehicle = vehicles[i]
        if DoesEntityExist(vehicle) then
            if GiveKeyssamy(source, vehicles[i], true) then
                success = true
            end
            Wait(20)
        end
...
RegisterNetEvent('qb-vehiclekeys:server:AcquireVehicleKeysamy', function(plate)
    giveKeyssamy(source, plate)
end)
